...
export default function Projects() {
  ...
  return (
    <section ref={ref} id="projects" className="relative py-20 md:py-28 px-4">
      ...
      {/* All modals always mounted — iframes preload in background, only selected is visible */}
      {projects.map((project) => (
        <ProjectDetail
          key={project.id}
          project={project}
          visible={selected?.id === project.id}
      ...
      <div className="p-5">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-[10px] mt-1" style={{ color: project.color }}>{project.subtitle}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.technologies.slice(0, 4).map((tech) => (
            <span key={tech} className="text-[9px] px-2 py-0.5 rounded bg-white/5 text-gray-300 border border-white/10">{tech}</span>
          ))}
...
function ProjectDetail({ project, visible, onClose }: { project: Project; visible: boolean; onClose: () => void }) {
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className={`absolute inset-0 bg-[#050505]/90 backdrop-blur-sm transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`} />

      <motion.div
        animate={visible ? { scale: 1, opacity: 1, y: 0 } : { scale: 0.9, opacity: 0, y: 30 }}
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3 border-b border-white/10 bg-[#0B1020]/95 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            <span className="font-pixel text-[10px]" style={{ color: project.color }}>NOW PLAYING</span>
        ...
        <div className="p-5 md:p-8 grid md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <p className="text-xs mb-4" style={{ color: project.color }}>{project.subtitle}</p>
            ...
            <DetailSection title="TECH STACK" color={project.color}>
              <div className="flex flex-wrap gap-2">
                {project.technologies.map((tech) => (
                  <span key={tech} className="text-[10px] px-2 py-1 rounded border" style={{ borderColor: `${project.color}40`, color: project.color }}>{tech}</span>
                ))}
              </div>
            </DetailSection>
...
function DetailSection({ title, color, children }: { title: string; color: string; children: React.ReactNode }) {
  return (
    <div className="mb-6">
      <h5 className="font-pixel text-[10px] mb-3" style={{ color }}>{title}</h5>
      {children}
    </div>
  );
}
